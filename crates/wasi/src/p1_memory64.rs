    ...
    fn clock_res_get(
        ...
        let res = match id {
            types::Clockid::Realtime => {
                let t = wall_clock::Host::resolution(&mut self.clocks())
    ...
    async fn fd_filestat_get(
        ...
        match desc {
            ...
            Descriptor::Directory { fd, .. } | Descriptor::File(File { fd, .. }) => {
                ...
                let atim = data_access_timestamp.unwrap_or(zero);
                let atim = types::Timestamp::from(atim.seconds)
                    .checked_mul(1_000_000_000)
                    .and_then(|ns| ns.checked_add(atim.nanoseconds.into()))
                    .ok_or(types::Errno::Overflow)?;
                let mtim = data_modification_timestamp.unwrap_or(zero);
                let mtim = types::Timestamp::from(mtim.seconds)
                    .checked_mul(1_000_000_000)
                    .and_then(|ns| ns.checked_add(mtim.nanoseconds.into()))
                    .ok_or(types::Errno::Overflow)?;
                let ctim = status_change_timestamp.unwrap_or(zero);
                let ctim = types::Timestamp::from(ctim.seconds)
                    .checked_mul(1_000_000_000)
                    .and_then(|ns| ns.checked_add(ctim.nanoseconds.into()))
                    .ok_or(types::Errno::Overflow)?;
                Ok(types::Filestat {
                    dev: 1,
