...
impl WasmtimeOptionValue for wasmtime::OmitBoundsChecks {
    const VAL_HELP: &'static str = "=none|all|dynamic";

    fn parse(val: Option<&str>) -> Result<Self> {
        match String::parse(val)?.as_str() {
            "none" => Ok(wasmtime::OmitBoundsChecks::None),
            "all" => Ok(wasmtime::OmitBoundsChecks::All),
            "dynamic" => Ok(wasmtime::OmitBoundsChecks::Dynamic),
            other => {
                bail!(
                    "unknown bounds checks omission strategy `{other}`, only none,all,dynamic accepted"
                )
            }
        }
    }
    ...
    fn display(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            wasmtime::OmitBoundsChecks::None => f.write_str("none"),
            wasmtime::OmitBoundsChecks::All => f.write_str("all"),
            wasmtime::OmitBoundsChecks::Dynamic => f.write_str("dynamic"),
        }
    }
}
